...
# Here, we implement direct RNN that return forecasted time series as output
    ...
    def __init__(self, nn_type, units, num_features, num_inputs,num_hidden,name,forecast_len=1):
        ...
        if (nn_type==6): # This is just a FNN We flatten the input s.t. the first num_inputs entries of the data represent the first time step, the second num_inputs entries of the data represent the second time step, and so on... this is from Grose & Watson
        ...
        if (nn_type==7): # Also a FNN but now we only care about the last time step. Taken from tensorflow
